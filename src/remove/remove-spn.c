...
void go(char *args, int alen) {
    ...
    BeaconPrintf(CALLBACK_OUTPUT, "[*] Starting remove SPN operation");
    BeaconPrintf(CALLBACK_OUTPUT, "[*] Target: %s %s", targetIdentifier, isTargetDN ? "(DN)" : "(name)");
    BeaconPrintf(CALLBACK_OUTPUT, "[*] SPN to remove: %s", spn);
    ...
    if (result == LDAP_SUCCESS) {
        ...
        BeaconPrintf(CALLBACK_ERROR, "[-] SPN '%s' not found on object", spn);
    } else {
